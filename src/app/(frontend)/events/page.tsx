...
import { IconCalendarEvent, IconClock, IconTicket, IconArrowRight } from '@tabler/icons-react';
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";

export default async function EventsPage() {
  // 1. Fetch live data from PayloadCMS
          ...
          <div className="max-w-5xl mx-auto rounded-2xl p-8 md:p-12 shadow-[0_8px_32px_rgba(15,23,42,0.06)] border border-white/50 bg-white/25 backdrop-blur-2xl backdrop-saturate-150 relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
              <div className="flex-1 space-y-6">
                ...
                <h2 className="text-4xl md:text-5xl font-black text-slate-900 leading-tight"> {nextFeaturedEvent.title} </h2>
                <p className="text-slate-600 text-lg leading-relaxed max-w-2xl"> {nextFeaturedEvent.description} </p>
                ...
                <div className="flex flex-wrap items-center gap-4 pt-2">
                  ...
                  <Badge variant="outline" className={`${getTagColor(nextFeaturedEvent.tag)} px-4 py-2 text-sm shadow-sm bg-white/60`}> {nextFeaturedEvent.tag} </Badge>
                </div>
              </div>

              <div className="shrink-0 w-full md:w-auto">
                <Button asChild size="lg" className="w-full md:w-auto bg-[#0b2447] hover:bg-blue-600 text-white rounded-xl py-7 px-10 text-lg font-bold shadow-xl shadow-blue-900/20 transition-all hover:-translate-y-1">
                  <Link href={`/events/${nextFeaturedEvent.slug}`}>
                    <IconTicket className="mr-2 h-6 w-6" /> More About Event
                  </Link>
                </Button>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {remainingUpcomingEvents.map((event) => (
                <Link href={`/events/${event.slug}`} key={event.id} className="block group">
                  <div className="bg-white/70 hover:bg-white transition-all duration-300 rounded-xl p-6 border border-white shadow-sm hover:shadow-md flex flex-col h-full">
          ...
          {pastEvents.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {pastEvents.map((event) => (
                <Link href={`/events/${event.slug}`} key={event.id} className="block group">
                  <div className="bg-slate-50/50 rounded-xl p-6 border border-slate-200/60 shadow-sm opacity-90 hover:opacity-100 transition-opacity flex flex-col h-full grayscale hover:grayscale-0">
                    <div className="flex justify-between items-start mb-4">
                      <Badge variant="outline" className={`${getTagColor(event.tag)} bg-white/50`}>{event.tag}</Badge>
                    </div>

                    <h3 className="text-lg font-bold text-slate-700 mb-2 group-hover:text-blue-600 transition-colors">{event.title}</h3>
                    <p className="text-slate-500 text-sm line-clamp-2 leading-relaxed mb-6 flex-grow">{event.description}</p>

                    <div className="flex items-center text-slate-400 text-xs font-medium mt-auto justify-between">
                      <div className="flex items-center">
                        <IconCalendarEvent size={14} className="mr-1.5" />
                        {formatDate(event.date)}
                      </div>
                      <IconArrowRight size={16} className="text-slate-300 group-hover:text-blue-500 transition-colors group-hover:translate-x-1" />
                    </div>
                  </div>
                </Link>
              ))}
